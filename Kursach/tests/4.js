...
TinyMCE_Engine.prototype = {
    ...
    , execCommand: function (command, user_interface, value) {
        ...
        switch (command) {
            case 'mceHelp': tinyMCE.openWindow(
                {
                    file: 'about.htm', width: 480, height: 380
                }

                ,
                {
                    tinymce_version: tinyMCE.majorVersion + "." + tinyMCE.minorVersion, tinymce_releasedate: tinyMCE.releaseDate, inline: "yes"
                }

            );
                return;
            case 'mceFocus': var inst = tinyMCE.getInstanceById(value);
                if (inst)
                    inst.contentWindow.focus();

                return;
            case "mceAddControl": case "mceAddEditor": tinyMCE.addMCEControl(tinyMCE._getElementById(value), value);
                return;
            case "mceAddFrameControl": tinyMCE.addMCEControl(tinyMCE._getElementById(value['element'], value['document']), value['element'], value['document']);
                return;
            case "mceRemoveControl": case "mceRemoveEditor": tinyMCE.removeMCEControl(value);
                return;
            case "mceResetDesignMode": if (!tinyMCE.isMSIE) {
                for (var n in tinyMCE.instances) {
    ...
    , handleEvent: function (e) {
        ...
        switch (e.type) {
            case "beforedeactivate": case "blur": if (tinyMCE.selectedInstance)
                tinyMCE.selectedInstance.execCommand('mceEndTyping');

                tinyMCE.hideMenus();
                return;
            case "drop": case "beforepaste": if (tinyMCE.selectedInstance)
                tinyMCE.selectedInstance.setBaseHREF(null);

                window.setTimeout("tinyMCE.selectedInstance.setBaseHREF(tinyMCE.settings['base_href']);", 1);
                return;
            case "submit": tinyMCE.removeTinyMCEFormElements(tinyMCE.isMSIE ? window.event.srcElement : e.target);
                tinyMCE.triggerSave();
                tinyMCE.isNotDirty = true;
                return;
            case "reset": var formObj = tinyMCE.isMSIE ? window.event.srcElement : e.target;
                ...
                return;
            case "keypress": if (inst && inst.handleShortcut(e))
                return false;
    ...
    , getButtonHTML: function (id, lang, img, cmd, ui, val) {
        var h = '', m, x;
        cmd = 'tinyMCE.execInstanceCommand(';
    ...
    , onLoad: function () {
        ...
        for (var c = 0; c < tinyMCE.configs.length; c++) {
            ...
            switch (mode) {
                case "exact": var elements = tinyMCE.getParam('elements', '', true, ',');
                    for (var i = 0; i < elements.length; i++) {
                ...
                case "specific_textareas": case "textareas": var nodeList = document.getElementsByTagName("textarea");
    ...
    , openWindow: function (template, args) {
        ...
        if (html) {
            ...
            var win = window.open("", "mcePopup" + new Date().getTime(),
                "top=" + y + ",left=" + x + ",scrollbars=" + scrollbars + ",dialog=yes,minimizable="
                + resizable + ",modal=yes,width=" + width + ",height=" + height + ",resizable=" + resizable);
            if (win == null) {
    ...
    , handleVisualAid: function (el, deep, state, inst) {
        ...
        switch (el.nodeName) {
            case "TABLE": var oldW = el.style.width;
            ...
            case "A": var anchorName = tinyMCE.getAttrib(el, "name");
                if (anchorName != '' && state) {
                    el.title = anchorName;
...
function TinyMCE_Control(settings) {
    ...
    this.cleanup.init(
        {
            valid_elements: s.valid_elements, extended_valid_elements: s.extended_valid_elements, entities: s.entities, entity_encoding: s.entity_encoding, debug: s.cleanup_debug, url_converter: 'TinyMCE_Cleanup.prototype._urlConverter', indent: s.apply_source_formatting, invalid_elements: s.invalid_elements, verify_html: s.verify_html, fix_content_duplication: s.fix_content_duplication
        }
...
TinyMCE_Control.prototype = {
    ...
    , addShortcut: function (m, k, d, cmd, ui, va) {
        ...
        sc = {
            alt: m.indexOf('alt') != -1, ctrl: m.indexOf('ctrl') != -1, shift: m.indexOf('shift') != -1, charCode: c, keyCode: n ? k : (ie ? c : null), desc: d, cmd: cmd, ui: ui, val: va
        };
    ...
    , execCommand: function (command, user_interface, value) {
        ...
        if (focusElm && focusElm.nodeName == "IMG") {
            ...
            switch (command) {
                case "JustifyLeft": if (align == 'left')
                ...
                case "JustifyCenter": img.removeAttribute('align');
                    ...
                    tinyMCE.triggerNodeChange();
                    return;
                case "JustifyRight": if (align == 'right')
                    img.removeAttribute('align');
                else
        ...
        if (tinyMCE.settings['force_br_newlines']) {
            ...
            if (doc.selection.type != "Control") {
                switch (command) {
                    case "JustifyLeft": alignValue = "left";
                        break;
                ...
                if (alignValue != "") {
                    ...
                    if ((divElm = tinyMCE.getParentElement(rng.parentElement(), "div")) != null)
                        ...
                        if (rng.pasteHTML && rng.htmlText.length > 0)
        ...
        switch (command) {
            ...
            case "Strikethrough": if (tinyMCE.isSafari)
            ...
            case "FormatBlock": if (value == null || value == "") {
            ...
            case "HiliteColor": if (tinyMCE.isGecko) {
            ...
            case "mceReplaceContent": if (!value)
                value = '';
            ...
            case "mceSetAttribute": if (typeof (value) == 'object') {
            ...
            case "mceSetCSSClass": this.execCommand("SetStyleInfo", false,
                {
                    command: "setattrib", name: "class", value: value
                }
            ...
            case "mceInsertContent": if (!value)
                value = '';
            ...
            case "mceStartTyping": if (tinyMCE.settings['custom_undo_redo'] && this.undoRedo.typingUndoIndex == -1) {
            ...
            case "mceEndTyping": if (tinyMCE.settings['custom_undo_redo'] && this.undoRedo.typingUndoIndex != -1) {
                this.execCommand('mceAddUndoLevel');
            ...
            case "mceAddUndoLevel": if (tinyMCE.settings['custom_undo_redo'] && this.undoRedoLevel) {
            ...
            case "Undo": if (tinyMCE.settings['custom_undo_redo']) {
                tinyMCE.execCommand("mceEndTyping");
                this.undoRedo.undo();
                tinyMCE.triggerNodeChange();
            }
            ...
            case "Redo": if (tinyMCE.settings['custom_undo_redo']) {
                tinyMCE.execCommand("mceEndTyping");
                this.undoRedo.redo();
                tinyMCE.triggerNodeChange();
            }
...
TinyMCE_Engine.prototype.convertSpansToFonts = function (doc) {
    var sizes = tinyMCE.getParam('font_size_style_values').replace('').split(',');
...
TinyMCE_Engine.prototype.convertFontsToSpans = function (doc) {
    ...
    if (fsClasses != '')
        fsClasses = fsClasses.replace('').split(',');
    else
        fsClasses = null;
...
function TinyMCE_Cleanup() {
    ...
    this.settings = {
        indent_elements: 'head,table,tbody,thead,tfoot,form,tr,ul,ol,blockquote,object', newline_before_elements: 'h1,h2,h3,h4,h5,h6,pre,address,div,ul,ol,li,meta,option,area,title,link,base,script,td', newline_after_elements: 'br,hr,p,pre,address,div,ul,ol,meta,option,area,link,base,script', newline_before_after_elements: 'html,head,body,table,thead,tbody,tfoot,tr,form,ul,ol,blockquote,p,object,param,hr,div', indent_char: '\t', indent_levels: 1, entity_encoding: 'raw', valid_elements: '*[*]', entities: '', url_converter: '', invalid_elements: '', verify_html: false
    };

    this.vElements = tinyMCE.clearArray(new Array());
    this.vElementsRe = '';
    this.closeElementsRe = '';
    this.codeElementsRe = '';
    this.serializationId = 0;
    this.mceAttribs = {
        href: 'mce_href', src: 'mce_src', type: 'mce_type'
    };
}
...
TinyMCE_Cleanup.prototype = {
    ...
    , parseRuleStr: function (s) {
        ...
        for (x = 0; x < ta.length; x++) {
            ...
            for (y = 0; y < tn.length; y++) {
                ...
                r.fill = px == '#';
                r.tag = r.tag.replace('');
                r.oTagName = tn[0].replace('').toLowerCase();
    ...
    , _serializeAttribute: function (n, r, an) {
        ...
        if (os && av.length != 0 && this.settings.url_converter.length != 0 && test(an))
    ...
    , xmlEncode: function (s) {
        ...
        switch (this.settings.entity_encoding) {
            case "raw": return tinyMCE.xmlEncode(s);
            case "named": for (i = 0, l = s.length; i < l; i++) {
                c = s.charCodeAt(i);
                e = this.entities[c];
                if (c == 39) {
                    o += "&#39;";
                ...
                return o;
            case "numeric": for (i = 0, l = s.length; i < l; i++) {
                c = s.charCodeAt(i);
...
TinyMCE_Engine.prototype.getElementByAttributeValue = function (n, e, a, v) {
    return this.getElementsByAttributeValue(n, e, a, v).length == 0 ? null : n[0];
};
...
TinyMCE_Engine.prototype.setInnerHTML = function (e, h) {
    ...
    if (tinyMCE.isMSIE && !tinyMCE.isOpera) {
        h = h.replace('>');
        h = h.replace('<p$1 mce_keep="true">&nbsp;</p>');
        h = h.replace('<p$1 mce_keep="true">&nbsp;</p>');
                ...
                if (typeof (attrib_name) == "undefined")
                    ...
                    if (node.getAttribute(attrib_name)) {
                        if (typeof (attrib_value) == "undefined") {
                            ...
                            if (node.getAttribute(attrib_name) == attrib_value)
                                return node;
                    }
            }
        }
    ...
    , getBookmark: function (simple) {
        ...
        if (tinyMCE.isMSIE) {
            ...
            if (rng.item) {
                ...
                return
                {
                    tag: e.nodeName; index: sp; scrollX: sx; scrollY: sy
                };
            }
            else {
                trng = rng.duplicate();
                ...
                return
                {
                    start: sp; length: le; scrollX: sx; scrollY: sy
                };
            }
        }
        ...
        if (tinyMCE.isGecko) {
            ...
            return
            {
                startTag: s.nodeName; start: sp; startIndex: si; endTag: e.nodeName; end: le; endIndex: ei
            };
        }
...
TinyMCE_Layer.prototype = {
    moveRelativeTo: function (re, p) {
        ...
        var x, y;
        switch (p) {
            case "tl": x = rep.absLeft;
                y = rep.absTop;
                break;

            case "tr": x = rep.absLeft + w;
                y = rep.absTop;
                break;

            case "bl": x = rep.absLeft;
                y = rep.absTop + h;
                break;

            case "br": x = rep.absLeft + w;
                y = rep.absTop + h;
                break;

            case "cc": x = rep.absLeft + (w / 2) - (ew / 2);
...
TinyMCE_Menu.prototype = tinyMCE.extend(TinyMCE_Layer.prototype,
    {
        init: function (s) {
            var n;
            this.settings = {
                separator_class: 'mceMenuSeparator', title_class: 'mceMenuTitle', disabled_class: 'mceMenuDisabled', menu_class: 'mceMenu', drop_menu: true
            };
        ...
        , update: function () {
            ...
            for (i = 0; i < m.length; i++) {
                ...
                switch (m[i].type) {
                    case 'separator': h += '<tr class="' + s.separator_class + '"><td>';
                        break;

                    case 'title': h += '<tr class="' + s.title_class + '"><td><span' + c + '>' + t + '</span>';
                        break;

                    case 'disabled': h += '<tr class="' + s.disabled_class + '"><td><span' + c + '>' + t + '</span>';
                        break;
